...
def create_genre_pie_chart():
    ...
    c.execute("SELECT genre, SUM(listeners) FROM soundcloud GROUP BY genre")
    ...
    plt.title('SoundCloud Top 100 Songs by Genre')
    plt.show()
...
def create_listener_pie_chart():
    ...
    c.execute("SELECT title, listeners FROM soundcloud ORDER BY listeners DESC")
    ...
    plt.pie(values, labels=labels, autopct='%1.1f%%')
    plt.title('SoundCloud Top 10 Most Listened to Songs')
    plt.show()
